...
function mostrarEstadoConexion(tipo, texto){
  if (!estadoConexionEl || !estadoTextoEl) return;
  estadoConexionEl.classList.remove('oculto', 'conectado', 'error');
  if (tipo) estadoConexionEl.classList.add(tipo);
...
function renderTodo(){
  ...
  if(!vistaActivaEl) return;
  const vistaActiva = vistaActivaEl.id;
  ...
  pedirConfirmacion(`Registrar pedido de ${cliente}: ${kg} kg (${formatoColones(total)}). ¿Confirmas?`, async ()=>{
    try{
      await db.collection(COLECCION_PEDIDOS).add({
        cliente, kg, precio: PRECIO_POR_KG, total, estado: 'pendiente',
        metodoPago: metodoPagoSeleccionado, nota: notaVentaInput.value.trim(), fecha: Date.now()
      });
      clienteInput.value = ''; kgInput.value = 1; notaVentaInput.value = '';
      metodoPagoVentaEl.querySelectorAll('.metodo-btn').forEach(b=>b.classList.remove('active'));
      metodoPagoVentaEl.querySelector('[data-metodo="Efectivo"]').classList.add('active');
      metodoPagoSeleccionado = 'Efectivo'; alertaFiado.classList.add('oculto');
      actualizarTotalPreview(); clienteInput.focus();
...
function diasDeAntiguedad(fecha){ return (Date.now() - fecha) / (1000 * 60 * 60 * 24); }
...
function renderPedidos(){
  ...
  if(filtrados.length === 0){
    ...
    pedidosVacioEl.textContent = pedidos.length === 0 ? 'Aún no hay pedidos registrados.' : 'No hay pedidos en este filtro.';
  ...
  filtrados.forEach((p, i)=>{
    ...
    card.innerHTML = `
      <div class="pedido-top">
        <div><div class="pedido-cliente">${escapeHtml(p.cliente)}</div><div class="pedido-fecha">${formatoFecha(p.fecha)}</div></div>
      ...
      <div class="badges">
        <span class="badge ${p.estado === 'pagado' ? 'badge-pagado' : 'badge-pendiente'}">${p.estado === 'pagado' ? 'Pagado' : 'Pendiente'}</span>
      ...
      <div class="pedido-acciones">
        <button class="btn-mini ${p.estado === 'pagado' ? 'pendiente-btn' : 'pagar'}" data-accion="toggle" data-id="${p.id}">${p.estado === 'pagado' ? 'Marcar pendiente' : 'Marcar pagado'}</button>
        <button class="btn-mini borrar" data-accion="borrar" data-id="${p.id}">✕</button>
      </div>
    `;
    card.addEventListener('click', (e)=>{ if(e.target.closest('button')) return; abrirDetalle(p.id); });
...
listaPedidosEl.addEventListener('click', (e)=>{
  ...
  if(btn.dataset.accion === 'toggle'){
    ...
    pedirConfirmacion(`¿Marcar el pedido de ${pedido.cliente} como "${nuevoEstado}"?`, async ()=>{
      try{ await db.collection(COLECCION_PEDIDOS).doc(id).update({ estado: nuevoEstado }); mostrarToast('Pedido actualizado', 'exito'); }catch(err){ console.error(err); mostrarToast('Error al actualizar el pedido', 'error'); }
    });
  }
  if(btn.dataset.accion === 'borrar'){
    pedirConfirmacion(`¿Seguro que deseas eliminar el pedido de ${pedido.cliente}?`, async ()=>{
      try{ await db.collection(COLECCION_PEDIDOS).doc(id).delete(); mostrarToast('Pedido eliminado', 'exito'); }catch(err){ console.error(err); mostrarToast('Error al eliminar el pedido', 'error'); }
    });
  }
});
...
function abrirDetalle(id){
  ...
  if(!p) return;
  idPedidoEditando = id;
  detCliente.value = p.cliente; detKg.value = p.kg; detNota.value = p.nota || '';
  metodoPagoDetalleEl.querySelectorAll('.metodo-btn').forEach(b=> b.classList.toggle('active', b.dataset.metodo === (p.metodoPago || 'Efectivo')));
  estadoDetalleEl.querySelectorAll('.metodo-btn').forEach(b=> b.classList.toggle('active', b.dataset.estado === p.estado));
...
detBtnSumar.addEventListener('click', ()=>{ detKg.value = (parseFloat(detKg.value || 0) + 0.5).toFixed(1).replace(/\.0$/, ''); });
detBtnRestar.addEventListener('click', ()=>{ detKg.value = Math.max(0.5, parseFloat(detKg.value || 0) - 0.5).toFixed(1).replace(/\.0$/, ''); });
metodoPagoDetalleEl.addEventListener('click', (e)=>{ const btn = e.target.closest('.metodo-btn'); if(!btn) return; metodoPagoDetalleEl.querySelectorAll('.metodo-btn').forEach(b=>b.classList.remove('active')); btn.classList.add('active'); });
estadoDetalleEl.addEventListener('click', (e)=>{ const btn = e.target.closest('.metodo-btn'); if(!btn) return; estadoDetalleEl.querySelectorAll('.metodo-btn').forEach(b=>b.classList.remove('active')); btn.classList.add('active'); });
detCancelar.addEventListener('click', ()=>{ modalDetalle.classList.add('oculto'); idPedidoEditando = null; });
detGuardar.addEventListener('click', ()=>{
  const cliente = detCliente.value.trim(); const kg = parseFloat(detKg.value);
  if(!cliente || !kg || kg <= 0){ mostrarToast('Revisa el cliente y los kilos', 'error'); return; }
  ...
  const nota = detNota.value.trim(); const total = kg * PRECIO_POR_KG;
  pedirConfirmacion('¿Seguro que deseas modificar este pedido?', async ()=>{
    try{ await db.collection(COLECCION_PEDIDOS).doc(idPedidoEditando).update({ cliente, kg, total, precio: PRECIO_POR_KG, metodoPago, estado, nota }); modalDetalle.classList.add('oculto'); idPedidoEditando = null; mostrarToast('Pedido actualizado', 'exito'); }catch(err){ console.error(err); mostrarToast('Error al guardar los cambios', 'error'); }
  });
});

/* ===== WhatsApp y PDF ===== */
document.getElementById('btn-whatsapp').addEventListener('click', ()=>{
  if(pedidos.length === 0){ mostrarToast('No hay pedidos para enviar', 'error'); return; }
  let texto = '*Lista de pedidos - Mamones*%0a%0a';
  pedidos.forEach(p=>{ texto += `${p.cliente} - ${p.kg}kg - ${formatoColones(p.total)} - ${p.estado === 'pagado' ? 'Pagado' : 'Pendiente'}%0a`; });
...
document.getElementById('btn-pdf').addEventListener('click', ()=>{
  ...
  doc.setFontSize(16); doc.text('Lista de Pedidos - Mamones', 14, 16);
  let y = 32; doc.setFontSize(11); doc.text('Cliente', 14, y); doc.text('Kg', 80, y); doc.text('Total', 105, y); doc.text('Pago', 140, y); doc.text('Estado', 170, y); y += 6; doc.line(14, y - 4, 196, y - 4);
  pedidos.forEach(p=>{ if(y > 280){ doc.addPage(); y = 20; } doc.text(String(p.cliente).substring(0, 26), 14, y); doc.text(String(p.kg), 80, y); doc.text(formatoColones(p.total), 105, y); doc.text(p.metodoPago === 'SinpeMovil' ? 'Sinpe' : 'Efectivo', 140, y); doc.text(p.estado === 'pagado' ? 'Pagado' : 'Pendiente', 170, y); y += 8; });
  doc.save('pedidos-mamones.pdf'); mostrarToast('PDF generado', 'exito');
});
...
document.getElementById('stats-aplicar').addEventListener('click', ()=>{
  filtroStats = { desde: statsDesde.value ? new Date(statsDesde.value + 'T00:00:00').getTime() : null, hasta: statsHasta.value ? new Date(statsHasta.value + 'T23:59:59').getTime() : null, estado: statsEstado.value };
  renderStats(); mostrarToast('Filtro aplicado', 'exito');
});
document.getElementById('stats-limpiar').addEventListener('click', ()=>{ filtroStats = null; statsDesde.value = ''; statsHasta.value = ''; statsEstado.value = 'todos'; renderStats(); });
...
function renderStats(){
  ...
  // Gráfico simple
  const dias = []; for(let i = 6; i >= 0; i--){ const f = new Date(); f.setHours(0,0,0,0); f.setDate(f.getDate() - i); dias.push(f); }
  const totalesPorDia = dias.map(fecha=>{ const sig = new Date(fecha); sig.setDate(sig.getDate() + 1); return { fecha, total: datos.filter(p => p.fecha >= fecha.getTime() && p.fecha < sig.getTime()).reduce((a,p)=>a + p.total, 0) }; });
  const maxDia = Math.max(1, ...totalesPorDia.map(d=>d.total));
  document.getElementById('stats-dias').innerHTML = totalesPorDia.map(d=>`<div class="dia-row"><div class="dia-label">${d.fecha.toLocaleDateString('es-CR', { weekday: 'short' })}</div><div class="dia-barra-fondo"><div class="dia-barra" style="width:${Math.round((d.total / maxDia) * 100)}%"></div></div><div class="dia-monto">${formatoColones(d.total)}</div></div>`).join('');
  // Clientes
  const porCliente = {}; datos.forEach(p=>{ const key = (p.cliente || '').trim(); if(!porCliente[key]) porCliente[key] = { kg: 0, total: 0 }; porCliente[key].kg += p.kg; porCliente[key].total += p.total; });
  const topClientes = Object.entries(porCliente).sort((a,b)=> b[1].total - a[1].total).slice(0, 5);
  document.getElementById('stats-clientes').innerHTML = topClientes.length === 0 ? '<p class="vacio">Sin datos.</p>' : topClientes.map(([nombre, d])=>`<div class="cliente-row"><span class="nombre">${escapeHtml(nombre)}</span><span class="info">${d.kg.toFixed(1).replace(/\.0$/, '')} kg · ${formatoColones(d.total)}</span></div>`).join('');
}
